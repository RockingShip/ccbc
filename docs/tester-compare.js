...
/*
 * Compare bezier curve/contour coordinate vector and determine error level
 */
...
function Bezier() {
	...
	this.calcControlsClosedOld2 = function(A, B, C)
	{
		let N = A.length, coef;
...
if (typeof window === "undefined") {
	...
	if (0) {
		...
		for (let i = 0; i < curve.AX.length; i++) {
			...
			curve.AY[i] = curve.AY[i] * 2160 / 500 * 2 - 200 - 500 - 100;
		}
	} else if (1) {
		// magnify
		width *= 2;
...
dt *= 8;
	...
	curve.compareInit(contourX.length*8, contourX, contourY);
	...
	do {
		...
		fs.writeFileSync("tester-compare-"+framenr.pad(3)+".png", buffer)
		framenr++;
		fs.writeFileSync("tester-compare.png", buffer)
